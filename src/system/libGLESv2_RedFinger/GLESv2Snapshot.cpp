...
GLSnapshotState::GLSnapshotState(GLESv2Decoder* gl) : mGL(gl) {
...
void GLSnapshotState::restore() {
    std::map<GLenum, bool>::iterator it1;
    for (it1 = mEnables.begin(); it1!=mEnables.end(); it1++) {
        if (it1->second) {
            mGL->glEnable(it1->first);
        } else {
            mGL->glDisable(it1->first);
        }
    }
    std::map<GLuint, GLuint>::iterator it2;
    for (it2 = mProgramNames.begin(); it2!=mProgramNames.end();it2++) {
        GLShaderState& shaderState = mShaderState[it2->first];
        it2->second = mGL->glCreateShader(shaderState.type);
        if (shaderState.source.size()) {
            GLint len = shaderState.source.size();
            const char* source = shaderState.source.c_str();
            const char** sources = &source;
            mGL->glShaderSource(it2->second, 1, sources, &len);
        }
        if (shaderState.compileStatus) {
            mGL->glCompileShader(it2->second);
        }
    }
