...
*/
#ifndef _GLES_V1_ENCODER_H_
#define _GLES_V1_ENCODER_H_
...
class GLESv1Encoder : public gles1_encoder_context_t {

public:
    GLESv1Encoder(IOStream *stream);
    virtual ~GLESv1Encoder();
    void initGL();
    void setClientState(GLClientState *state) {
        m_state = state;
    }
    void setSharedGroup(GLSharedGroup* shared) {
        m_shared = shared;
        if (m_state && m_shared)
            m_state->setTextureData(m_shared->getTextureData());
    }
    const GLClientState *state() { return m_state; }
    GLClientState *state_static() { return m_state; }
    const GLSharedGroup* shared() { return m_shared; }
    GLSharedGroup *shared_static() { return m_shared; }
    ...
    gles1_encoder_context_t* GetContext(){
        return (dynamic_cast<gles1_encoder_context_t*>(this));
    }
private:

    bool    m_initialized;
    GLClientState* m_state;
    GLSharedGroup* m_shared;
    GLenum  m_error;
