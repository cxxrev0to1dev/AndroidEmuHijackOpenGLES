...
egl_dispatch *create_egl_dispatch(void *gles_android)
{
    ...
    ptr = dlsym(gles_android,"eglGetProcAddress"); disp->set_eglGetProcAddress((eglGetProcAddress_t1)ptr);
    ...
    //const char* hardware_gles = "/system/lib/libEGL.so";
    //gles_android = dlopen(hardware_gles, RTLD_NOW | RTLD_LOCAL);

    ptr = dlsym(gles_android,"eglWaitSyncKHR");
    if(!ptr){
        ptr = (void*)disp->eglGetProcAddress("eglWaitSyncKHR");
    }
    disp->set_eglWaitSyncKHR((eglWaitSyncKHR_t)ptr);
    ptr = dlsym(gles_android,"eglGetRenderBufferANDROID");
    if(!ptr){
        ptr = (void*)disp->eglGetProcAddress("eglGetRenderBufferANDROID");
    }
    disp->set_eglGetRenderBufferANDROID((eglGetRenderBufferANDROID_t)ptr);
    ptr = dlsym(gles_android,"eglDupNativeFenceFDANDROID"); 
    if(!ptr){
        ptr = (void*)disp->eglGetProcAddress("eglDupNativeFenceFDANDROID");
    }
    disp->set_eglDupNativeFenceFDANDROID((eglDupNativeFenceFDANDROID_t)ptr);
    ptr = dlsym(gles_android,"eglRenderBufferModifiedANDROID");
    if(!ptr){
        ptr = (void*)disp->eglGetProcAddress("eglRenderBufferModifiedANDROID");
    }
    disp->set_eglRenderBufferModifiedANDROID((eglRenderBufferModifiedANDROID_t)ptr);
    ptr = dlsym(gles_android,"eglGetSystemTimeFrequencyNV"); 
    if(!ptr){
        ptr = (void*)disp->eglGetProcAddress("eglGetSystemTimeFrequencyNV");
    }
    disp->set_eglGetSystemTimeFrequencyNV((eglGetSystemTimeFrequencyNV_t)ptr);

    ptr = dlsym(gles_android,"eglGetSystemTimeNV");
    if(!ptr){
        ptr = (void*)disp->eglGetProcAddress("eglGetSystemTimeNV");
    } 
    disp->set_eglGetSystemTimeNV((eglGetSystemTimeNV_t)ptr);
    ptr = dlsym(gles_android,"eglHibernateProcessIMG");
    if(!ptr){
        ptr = (void*)disp->eglGetProcAddress("eglHibernateProcessIMG");
    }
    disp->set_eglHibernateProcessIMG((eglHibernateProcessIMG_t)ptr);
    ptr = dlsym(gles_android,"eglAwakenProcessIMG");
    if(!ptr){
        ptr = (void*)disp->eglGetProcAddress("eglAwakenProcessIMG");
    }
    disp->set_eglAwakenProcessIMG((eglAwakenProcessIMG_t)ptr);

    ptr = dlsym(gles_android,"eglSetBlobCacheFuncsANDROID");
    if(!ptr){
        ptr = (void*)disp->eglGetProcAddress("eglSetBlobCacheFuncsANDROID");
    }
    disp->set_eglSetBlobCacheFuncsANDROID((eglSetBlobCacheFuncsANDROID_t)ptr);
    return disp;
}
