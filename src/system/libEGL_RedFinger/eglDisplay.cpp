...
bool eglDisplay::initialize()
{
    ...
    if (!m_initialized) {
        if (!getEGLThreadInfo()->EGLInfo) {
            pthread_mutex_unlock(&m_lock);
            ALOGE("Failed to establish connection with the host\n");
            return false;
        }
        getEGLThreadInfo()->EGLInfo->setGrallocOnly(false);

        //
        // get renderControl encoder instance
        //
        renderControl_encoder_context_t *rcEnc = getEGLThreadInfo()->EGLInfo->GetRenderControlEncoderContext();
        if (!rcEnc) {
            pthread_mutex_unlock(&m_lock);
...
static char *buildExtensionString()
{
    ...
    if (n > 0) {
        ...
        if (getEGLThreadInfo()->EGLInfo->GetRenderControlEncoderContext()->hasNativeSync() &&
