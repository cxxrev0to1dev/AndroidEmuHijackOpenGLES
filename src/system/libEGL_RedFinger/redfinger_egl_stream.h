...
*/
#ifndef __REDFINGER_EGL_STREAM_H_
#define __REDFINGER_EGL_STREAM_H_

#include <stdlib.h>
#include "IOStream.h"


class RedFingerEGLStream : public IOStream {
public:
    typedef enum { ERR_INVALID_SOCKET = -1000 } TestError;

    explicit RedFingerEGLStream(size_t bufsize = 1024*1024*20/*10000*/);
    virtual ~RedFingerEGLStream();
...
private:
    ...
    RedFingerEGLStream* writeback_class_;
protected:
    int            m_sock;
    size_t         m_bufsize;       
    unsigned char *m_buf;
    ...
    RedFingerEGLStream(int sock, size_t bufSize);
};
